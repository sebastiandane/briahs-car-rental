...
const downtime = [
  { m: "Jan", days: 18 }, { m: "Feb", days: 14 }, { m: "Mar", days: 22 },
  { m: "Apr", days: 12 }, { m: "May", days: 9 },  { m: "Jun", days: 16 },
];
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={<Btn variant="primary"><Plus className="h-4 w-4" /> Schedule service</Btn>}
      />

      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <KPI accent label="Overdue"     value={String(overdue)}    delta="Act now"    icon={<AlertTriangle className="h-4 w-4" />} />
        <KPI       label="In progress"  value={String(inProgress)} icon={<Wrench className="h-4 w-4" />} />
        <KPI       label="Scheduled"    value={String(scheduled)}  icon={<CalendarCheck className="h-4 w-4" />} />
        <KPI       label="Service spend (mo)" value={peso(totalCost)} icon={<Activity className="h-4 w-4" />} />
      </div>
            ...
            <tbody>
              {maintenance.map((m) => (
                <tr key={m.id} className="border-b border-border/60 hover:bg-secondary/40">
                  ...
                  <td className="px-4 py-3 text-right font-display font-semibold">{peso(m.cost)}</td>
          ...
          <div className="h-64 p-4">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={downtime}>
                ...
                <XAxis dataKey="m" tick={{ fill: "oklch(0.72 0.015 250)", fontSize: 11 }} axisLine={{ stroke: "rgba(255,255,255,0.06)" }} tickLine={false} />
                <YAxis tick={{ fill: "oklch(0.72 0.015 250)", fontSize: 11 }} axisLine={{ stroke: "rgba(255,255,255,0.06)" }} tickLine={false} />
