...
}: { label: string; value: string; delta?: string; icon: ReactNode; accent?: boolean }) {
  return (
    <div className={`relative overflow-hidden rounded-xl border ${accent ? "border-primary/30 bg-gradient-to-br from-card to-primary/10" : "border-border bg-card"} p-5 shadow-soft`}>
      <div className="flex items-start justify-between">
        ...
        <span className="grid h-9 w-9 place-items-center rounded-md bg-primary/15 text-primary">{icon}</span>
      </div>
    </div>
  );
}
...
const badgeMap: Record<string, string> = {
  ...
  // payment
  "Partially Paid": "bg-amber-500/15 text-amber-400 border-amber-500/30",
...
export function Toolbar({ children }: { children: ReactNode }) {
  return <div className="mb-5 flex flex-wrap items-center gap-3 rounded-xl border border-border bg-card p-3">{children}</div>;
}
...
export function TInput(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      {...props}
      className={`h-9 rounded-md border border-border bg-background px-3 text-sm text-foreground outline-none placeholder:text-muted-foreground focus:border-primary ${props.className ?? ""}`}
    />
  );
}
...
export function TSelect(props: React.SelectHTMLAttributes<HTMLSelectElement>) {
  return (
    <select
      {...props}
      className={`h-9 rounded-md border border-border bg-background px-3 text-sm text-foreground outline-none focus:border-primary ${props.className ?? ""}`}
    />
  );
}

export function Btn({ children, variant = "default", ...rest }: { children: ReactNode; variant?: "default" | "ghost" | "primary" | "danger" } & React.ButtonHTMLAttributes<HTMLButtonElement>) {
  const map = {
  ...
  return (
    <button {...rest} className={`inline-flex h-9 items-center gap-1.5 rounded-md px-3 text-sm font-medium transition-colors ${map[variant]} ${rest.className ?? ""}`}>
      {children}
    </button>
  );
}
